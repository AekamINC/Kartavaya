...
const TABS = ['overview', 'content', 'generate', 'chat', 'knowledge', 'publish', 'brand', 'credits', 'skills'];
...
const PLATFORMS = [
  ...
  },
  {
    ...
    supports: ['Tweets (280 chars)', 'Threads'],
    manualOnly: true,
  },
  {
  ...
  },
  {
    ...
    supports: ['Text messages', 'Photo messages', 'HTML formatting'],
    manualOnly: true,
  },
  {
    ...
    supports: ['Stories', 'Spotlight posts'],
    manualOnly: true,
  },
  {
...
function PublishTab({ clientId }) {
  ...
  useEffect(() => {
    ...
    if (params.get('oauth') === 'success') {
      pushToast({ title: `${params.get('platform') || 'Platform'} connected successfully`, type: 'success' });
  ...
  async function loadEnabledPlatforms() {
    try {
      ...
      setEnabledPlatforms(PLATFORMS.map(p => p.key));
    }
  }
    ...
    try {
      if (bulkAccounts.length > 1) {
        ...
        await api.post(`/v1/hub/clients/${clientId}/publish/schedule`, {
          ...scheduleForm,
          social_account_id: bulkAccounts[0] || scheduleForm.social_account_id,
        });
    ...
    <div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
          {PLATFORMS.filter(p => !enabledPlatforms || enabledPlatforms.includes(p.key)).map(p => {
            ...
            return (
              <div key={p.key} style={{ background: 'var(--surface-1)', border: `1px solid ${isConnected ? p.color + '40' : 'var(--rule-soft)'}`,
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  <span style={{ width: 36, height: 36, borderRadius: 8, background: p.color, color: '#fff',
                ...
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em',
                    color: 'var(--ink-4)', marginBottom: 6 }}>Prerequisites</div>
                  {p.prereqs.map((req, i) => (
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em',
                    color: 'var(--ink-4)', marginBottom: 4 }}>Supports</div>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {p.supports.map((s, i) => (
                      <span key={i} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 99,
                        background: 'var(--surface-0)', color: 'var(--ink-3)' }}>{s}</span>
                    ))}
                  </div>
                </div>
                ...
                {connected.map(a => (
                  <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    background: `${p.color}08`, border: `1px solid ${p.color}20`, borderRadius: 8,
                ...
                {showManual === p.key && (
                  <form onSubmit={e => connectManual(e, p.key)}
                    ...
                    <div style={{ display: 'grid', gap: 8 }}>
                      ...
                      <input className="k-input" type="password" placeholder="Access Token" required value={manualForm.access_token}
                    ...
                    <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', marginTop: 8 }}>
                      <button type="button" className="k-btn k-btn--ghost" style={{ fontSize: 11 }}
                        onClick={() => { setShowManual(null); setManualForm({ account_name: '', account_id: '', page_id: '', access_token: '' }); }}>
                        Cancel
                      </button>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <select className="k-input" style={{ width: 130, fontSize: 12 }} value={queueFilter}
        ...
        {showSchedule && (
          <form onSubmit={schedulePost}
            style={{ background: 'var(--surface-1)', border: '1px solid var(--rule-soft)', borderRadius: 12, padding: 24, marginBottom: 12 }}>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {accounts.map(a => {
                  const pl = PLATFORMS.find(p => p.key === a.platform);
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              <button type="button" className="k-btn k-btn--ghost"
                onClick={() => { setShowSchedule(false); setBulkAccounts([]); }}>Cancel</button>
        ...
        {queue.filter(q => !queueFilter || q.status === queueFilter).length === 0 ? (
          <p style={{ color: 'var(--ink-3)', fontSize: 13, padding: 24, textAlign: 'center' }}>
            No posts in queue{queueFilter ? ` with status "${queueFilter}"` : ''}.
          </p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {queue.filter(q => !queueFilter || q.status === queueFilter).map(q => {
              const pl = PLATFORMS.find(p => p.key === q.platform);
              return (
                <div key={q.id} style={{ background: 'var(--surface-1)', border: '1px solid var(--rule-soft)', borderRadius: 10, padding: '12px 16px' }}>
                  ...
                  {q.status === 'scheduled' && (
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="k-btn k-btn--primary" style={{ fontSize: 12, padding: '4px 12px' }}
                        onClick={() => publishNow(q.id)}>Publish Now</button>
                      <button className="k-btn k-btn--ghost" style={{ fontSize: 12, padding: '4px 12px', color: '#ef4444' }}
                        onClick={() => cancelPost(q.id)}>Cancel</button>
                    </div>
                  )}
                  {q.platform_url && (
