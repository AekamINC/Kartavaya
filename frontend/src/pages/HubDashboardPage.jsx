...
const TABS = ['generate', 'content', 'chat', 'knowledge', 'publish', 'brand', 'credits', 'clients'];
...
export default function HubDashboardPage() {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px 48px' }}>
      ...
      {tab === 'clients' && <ExternalClientsTab />}
    </div>
  );
}
...
function ExternalClientsTab() {
  ...
  async function load() {
    try {
      ...
      setClients((r.data.data || []).filter(c => !c.is_internal));
    } catch {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--ink-2)' }}>Manage external brand clients for agency work.</p>
      ...
      {clients.length === 0 ? (
        <div style={{ background: 'var(--surface-1)', border: '1px solid var(--rule-soft)', borderRadius: 12, padding: 48, textAlign: 'center' }}>
          <p style={{ color: 'var(--ink-3)', fontSize: 14 }}>No external clients. Create clients here if you manage content for other brands.</p>
        </div>
      ) : (
