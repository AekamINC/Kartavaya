 ...
 *
 * ── WHAT THIS IS NOW, AFTER `28-messaging-v2.md` §2 ─────────────────────────
 *
 * It is no longer the only way to read a reply, and it is no longer a column.
 * §2 in as many words: "**Do not delete `ThreadPanel`.** It stays as the mobile
 * presentation of the same data — a phone has no room to indent — and it is
 * what a deep link to a reply still opens. What goes away is it being the
 * *only* way to read a reply."
 *
 * So: above 767px a thread expands INSIDE the log (`InlineThread`, `.m2th`), and
 * below it the same tap opens this. `ChatPane` owns the one piece of state that
 * decides which — `openThreadId` — and renders this as an overlay inside its own
 * `.m2c` column, absolutely positioned by a rule `sanvaad.css` already declares
 * at :2428 for every width at or under 900px. It is NOT rendered by
 * `ChannelsTab` any more and is not a grid track; `.sv--thread` is gone.
 */
...
/**
 * There is still no `onReplied`, and the reason has changed shape — so it is
 * restated rather than left standing as it was.
 *
 * WHAT WAS TRUE AND IS NOT ANY MORE: this panel used to be a SIBLING of
 * `ChatPane`, two columns of a grid `ChannelsTab` owned, so no handler could
 * reach the messages without being threaded through a component that had no
 * other reason to hold them. It is now a CHILD of `ChatPane`, which is the file
 * that calls `useChannelMessages`. The wiring is one prop away. Saying it is
 * impossible would be false.
 *
 * WHAT IS STILL TRUE: nothing passes one, so the count is kept honest by
 * `useChannelMessages`'s own poll, which re-reads `thread_count` and
 * `last_reply_at` off `list_messages` a few seconds later. The one path that
 * beats the poll is a reply sent from the CHANNEL composer, because that goes
 * through the hook (`send(content, parentId)` bumps the parent's `thread_count`
 * itself) — this one posts directly and cannot reach that state. So a reply sent
 * from inside the panel leaves the disclosure behind it reading "2 replies" for
 * up to one poll. That is a known gap with a known ceiling, not a claim that it
 * cannot be closed.
 */
export default function ThreadPanel({
  ...
  /**
   * The channel's members — the same list `ChatPane`'s hook already holds,
   * handed straight down rather than fetched a second time here. (It used to
   * travel up through `ChannelsTab` and back down; that lift is gone with the
   * grid column.) It is the mention vocabulary for the replies below AND for the
   * composer at the foot of this panel, and until it arrived neither had one.
   */
  members = EMPTY,
}) {
  ...
  return (
    /* `svThreadIn` / `svThreadOut` live on this element; `ChatPane` owns the
       `closing` flag and the unmount, through `useExitAnimation` — which is
       also what filters an animation bubbling up from a skeleton or a reply
       inside this panel out of the exit's `animationend`. `aria-hidden` while
