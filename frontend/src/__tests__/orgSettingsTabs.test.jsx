...
describe('Organisation hub tab bar', () => {
  // TWELVE, and each addition is a decision rather than a drift. The design had
  ...
  // either. `Reports` is the sixth, added 2026-08-29 with the encrypted-PDF
  // report delivery: the scheduled Finance and CRM reports now travel as a
  // password-protected PDF, the passphrase is NEVER in the email, and this tab
  // is the only place a recipient can be told what it is — the report email
  // names it by this exact path.
  //
  // Recycle bin sits between Storage and Danger zone on purpose: Storage is
  // what this organisation is using and the bin is the part of that figure it
  // can still get back, since binned files count against the allowance at both
  // stages. Reports sits beside Security rather than inside it, because
  // `TabSecurity` gates every control on a probe for `staging.org_security` and
  // hiding the report passphrase behind an unrelated table's existence is how
  // one missing migration breaks two features.
  //
  ...
  it('renders the tabs in the design\'s order, plus the six added since', async () => {
    await draw();
    expect(tabs().map(t => t.textContent.replace(/[^A-Za-z ]/g, '').trim())).toEqual([
      'Profile', 'Members', 'Billing', 'Modules', 'Compliance', 'Senders',
      'UPI IDs', 'Security', 'Reports', 'Storage', 'Recycle bin', 'Danger zone',
    ]);
  });
  ...
  it('carries each tab\'s Devanagari from the designer\'s TAB_HI map', async () => {
    ...
    //
    // प्रतिवेदन is taken the same way and for the same reason: it is already
    // this product's word for a report, in the three report emails
    // `email_service.py` sends — "दैनिक प्रतिवेदन", "साप्ताहिक प्रतिवेदन",
    // "मासिक प्रतिवेदन". The tab and the email a recipient is holding must not
    // say the object's name two different ways, or they read as two features
    // and the mail's instruction to open this screen stops being followable.
    expect(tabs().map(t => t.querySelector('.tabs__hi')?.textContent))
      .toEqual(['रूपरेखा', 'सदस्य', 'बीजक', 'खंड', 'अनुपालन', 'प्रेषक',
                'यूपीआई', 'सुरक्षा', 'प्रतिवेदन', 'भंडार', 'रद्दी', 'संकट']);
  });
