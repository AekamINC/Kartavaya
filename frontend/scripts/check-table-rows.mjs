...
const EXEMPT = {
  lgl__table: 'the public sub-processor list — a document table on the legal pages, not a record list. Every cell is a wrapping sentence ("What it does", "Data it receives"), so the row height is set by the longest prose in it: a fixed 66px would either clip the description or leave the short rows hollow. The token exists to give a comfortable hit target on a row you act on, and nothing in this table is clickable, selectable or editable. It also has to print — legal.css strips the chrome for Ctrl-P so that counsel at a prospective customer can attach a PDF — and a forced row height paginates a long description across a page break.',
