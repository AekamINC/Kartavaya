...
## 🟡 `pahchan.attendance_by_shift` stays absent — the conclusion holds, the reason did not

Checked 2026-09-07 after `late_arrivals` turned out to have a false absence reason. **This one's conclusion is correct and the metric should stay absent**, but two of its stated grounds were wrong:

- ⚠ *"(027, 12 rows live)"* about `manav_shift_definitions` — **stale**. It holds **0** today; migration 260 cleared every non-Aekam row. The reason was arguing from a number that had been false for a week, and happened to reach the right answer anyway, which is why nobody caught it.
- ⚠ *"joining a same-day `manav_schedules` row would silently drop every attendance day the optional scheduler never covered"* — that describes an INNER join. A `LEFT JOIN` with `COALESCE(name, 'Unscheduled')` drops nothing, and is exactly the pattern this module already uses for `'No department'`.

**What actually blocks it**, re-measured against `information_schema`:

- Neither `manav_attendance` nor `pahchan_punches` has **any** column matching `%shift%`. That is structural and decisive.
- `manav_employees.shift` is `text DEFAULT 'general'` with no FK — a property of the **person**, not the day. This module models a shift as changing daily (schedules, bids, swaps), so bucketing historical attendance by somebody's *current* label would silently re-file their past whenever they moved shift. `department` shares that flaw in principle but not in practice.
- `manav_schedules` **does** record `(employee_id, date, shift_id)` with a real FK, and is the one honest source — but joining it needs `employee_id` a **second** time, and the DPDP pin allows exactly one occurrence (the department join). Widening a privacy ratchet to gain a dimension is an owner's decision, not a refactor. With 0 schedules and 0 shift definitions live, it would today yield a single `'Unscheduled'` bucket anyway.

**A new ratchet came out of this.** `test_absent_reasons_may_not_rest_on_a_row_count` fails any pahchan absence citing `N rows` — a row count is a measurement with a date, and it is the one part of such a claim that goes false without anybody editing the file. Proved by restoring the real `(027, 12 rows live)` clause: the test names it and fails.
