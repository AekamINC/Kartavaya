...
| **0.27** | **DECIDED — seed the rate card with ESTIMATE data.** Meta's own INR card cannot be obtained: it sits behind their Business Manager login, and entering credentials is not something this seat does. Owner's answer: *"do an e2e seed with estimate data."* So seed published India per-conversation pricing, **stamped as an estimate on the row**, so pricing, margin and the free-entry-point flow can all be exercised end to end before the real card exists. The estimate must be visibly an estimate wherever it surfaces — an unmarked guess about what a customer will be charged is worse than no number. Replace on 0.26, when the owner connects the account. |
