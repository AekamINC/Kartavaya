...
| 3 | Billing executable + arm cron | 🟡 **3.1 ✅ · 3.2 ✅ shipped, UI acceptance owed · 3.3 ✅ ACCEPTANCE PASSED on live rows · 3.4 verified but not scheduled.** 3.3 is the one with rows behind it: `/cron/billing` fired twice against the deploy on 26 Aug — **`client_invoice_lines` 0 → 2 and auto-invoices 0 → 2**, a loop that had never once produced a document. `INV-2026-0093` ₹75,000 + ₹13,500 = **₹88,500** and `INV-2026-0094` ₹15,000 + ₹2,700 = **₹17,700**, both for 2026-08-01 – 2026-09-01, intra-state Gujarat (CGST+SGST, `place_of_supply` 24), `unpaid`, `balance_due = total`, line bodies populated. **Second run: `created 0, skipped 2`** — no double-billing. April–July were NOT raised: `invoice_from` held. The fault it fixes — the period was recomputed from the service line's **origin** every run, so a monthly retainer invoiced **once, for ever**, and said `skipped`. 3.2 — a mid-cycle plan change raised **two debits**; the credit is now a `kind='credit'` line (migration 222, verified from `pg_constraint`) that every total, the preview and `invoice_billing_lines` subtract, and `proration.py`'s day-count (31 days for August 2026 against payroll's 26) is unified on 0.17. Its acceptance — a real plan change through the console — is still owed. Live-parsed against the real schema: `test_billing_credit_sql_is_valid.py` 7, `test_client_billing_invoices.py` 37, both under `railway run`; offline `test_proration.py` 23, `test_billing_lines.py` 84. **3.4: `billing` still needs adding to `cron-daily`'s loop** — see above |
| 4 | Eight invisible-feature screens | 🟡 **six of eight landed 27 Aug — every one an endpoint that had no caller at all.** **4.1** compliance settings: `module_compliance_settings` held 0 rows across all five orgs and nothing called `/v1/org/compliance`; the screen records not-applicable/applicable/enforced per rule with the consequence stated before the confirm, and the never-claim rule is structural — `enforced` is refused for a rule no code reads, and two tests hold it. **4.4** storage browser: 390 lines and 19 tests with no caller; wiring it surfaced two real faults — `resolve` matched **none of the 137 stored keys** because it only tried the new grammar, and the folder names are `personal/user_…`, `pahchan/{uuid}`, `projects/team_…` so drawing them breaks names-not-ids on the first click. **4.5** the dock Due tab: `DUE_SOURCE` was null and **two** tests pinned the dead state shut (deleted, not skipped); the `income_tax`/`incometax` typo was worth **22 rows** and Finance goes 7 → 13 obligations. **4.6/4.7/4.8** the billing anchor, pause/resume and quota proration controls. **4.2** Pahchan consent: 12 faces enrolled and ZERO consents ever recorded, with no other way to be marked present if you declined; `method='self_acknowledged'` is a value migration 209 always admitted and no route could write. **4.3 in flight.** ⚠ Two of these are ✅ only when a row appears: compliance settings is still **0 rows** (one click on staging after deploy) and `skill_finding_ack` is still 0. Storage and Due are read-only surfaces with no row to write |
...
| 6 | Retire 4 duplicate models + SQL-test rule | 🟡 **the rule is shipped and two of the four "duplicates" turned out not to be, one of them dangerously.** ⚠ **`pay_professional_tax` is NOT part of the dead `pay_*` stack** — it holds **9 live rows**, is the shared PT ladder every payroll run reads, and migration 221 extended it last week; the plan's "drop the `hr_*`/`pay_*` stack" would have taken professional tax to **₹0 for every employee**. The other 17 are genuinely empty. 6.4 is a stale premise: `report_schedules` **does not exist**, so there is one scheduler, not two. 6.3 decided — KEEP BOTH allocators, because a PO is numbered at ISSUE and `next_doc_number`'s `ORDER BY created_at` would restart the series at 0001 on the next draft; the boundary is now `test_two_serial_allocators.py` (5). **The process rule is a ratchet**: `test_every_writer_has_a_live_sql_test.py` (4) — 36 routers write to `staging.*`, 6 have a live-schema test, 30 baselined and the baseline only shrinks. 6.1's three commission tables confirmed 0/0/0 and still await the owner's OK to drop (0.30). Counts are exact `count(*)` — `n_live_tup` reports 0 for both live tables and is not usable here |
