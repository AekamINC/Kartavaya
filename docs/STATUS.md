...
| Report delivery passphrase | 93 | 🟡 **code-without-data, measured 30 Aug.** Org settings → **Reports** shipped and is live: verified in the deployed Pages bundle (`index-Dxmt9ulU.js`), not just in the repo. `GET`/`PUT /api/v1/org/profile/report-passphrase` exists (`org_profile.py:1428`) and `tests/test_report_passphrase_sql.py` parses its SQL against the real schema. It is 🟡 and **not** ✅ because **no organisation has set one**: `settings->'reports'` is absent on **all 5** orgs, so `load_passphrase` returns `''` for every one and every scheduled report still leaves in the link shape rather than as an encrypted PDF. The tab is its own, not a row inside Security, because `TabSecurity` gates every control on a probe for `org_security` — one missing migration would otherwise take two features down |
