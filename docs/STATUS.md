...
control in the product (chrome, core, settings, money, people, crm, comms).

✅ **The result worth stating is what PASSED.** For a suite whose whole purpose
is finding controls that do nothing: every sweep clean, **no control answered a
5xx, none answered a 4xx**, and the protected Aekam Inc team untouched. Its
preflight also records `outbound_mode=live`, so **every send control was
excluded by name** — nothing was clicked that would reach a real person.

🔴 **22.93 — controls covered by something on top of them**, in three classes:

| Blocked | By | Where |
|---|---|---|
| "Next page", "Open", "Edit" | `button.k-dock__pill` | 4 screens across graha + ganit |
...
**Measured:** the corner dock's pill is 38px tall sitting 20px off the bottom, so
**the dock owns the bottom 58px of the viewport while `.mpage` reserves 20px**.
That mismatch is the defect. (`module.css` declares 48px there and it computes
to 20px — something overrides it, and that wants finding first.)

⚠ **NOTHING WAS SHIPPED FOR THIS, DELIBERATELY.** The overlap could not be
reproduced with today's data — short list, inner scroll container, hit test found
**0** covered — and a padding change across every page that cannot be shown to
fix anything is exactly the guess this session twice avoided. Where proof WAS
available (the row heights) the rules were injected live and moved 24 → 0 and
18 → 0. Here it was not, so the arithmetic is recorded for the next attempt
instead. The `th.tbl__th--rz` self-block is the most suspicious of the three and
is not yet diagnosed.

---

## 2026-08-31 — ✅ 20.05 MEASURED AND BOTH CAUSES FIXED, AND THE TDS VERDICT TAKEN

**The six genuine `--row-h` violations are explained and fixed** — measured with
a throwaway probe that hid each cell until the row fell back to the token,
rather than guessed at with a global `white-space: nowrap`.

⚠ **AND THE FIRST PASS GOT IT BACKWARDS.** It reported "no cell is wrapping"
because `scrollWidth === clientWidth` on every cell. That is exactly what a
WRAPPED cell looks like — wrapping is what removes the horizontal overflow. The
check that appears to prove single-line proves the opposite.

| Screen | Cause | Fix |
|---|---|---|
| `graha#contacts` 59.5px | the PHONE cell wraps to 3 lines at 71px wide | `.gr__td--mute { white-space: nowrap }` — **no ellipsis**; half a phone number is not a phone number |
| `vikray#stock` 56.5px | the threshold control is **55.5px stacked** — no 50px row can hold it | laid out as a row; the column is 356px wide against an 82px input |

**Proved against the LIVE app** by injecting each rule and re-counting:
`graha#contacts` **24 → 0**, `vikray#stock` **18 → 0**. A third candidate
changed nothing and was discarded. `ganit#contacts` / `vikray#contacts` share
`graha/ContactsTab`, so the first rule covers them.

### ✅ THE TDS/GSTIN VERDICT — the rule is about CAPTURE, not EMISSION

Suite 05.17 found a TDS challan refused for a missing TAN, named the tension
with the standing rule that *"GSTIN / PAN / TAN … must block nothing"*, and
**deliberately refused to rule** (93 §14). Taken now, and written into
`CLAUDE.md` so the next reader does not "fix" the validator:

**`validate_tds_challan` and the GSTR-1 `supplier_gstin_missing` refusal are
CORRECT and must not be relaxed.** Nothing may *require* those numbers to save a
client, invoice or employee — and `org_profile.py:462` carries the owner's own
words to that effect, while `VendorForm.jsx:297` confirms they block no save. But
an ITNS-281 without a TAN is invalid under **s.203A**, where a PAN is explicitly
not a substitute, and a GSTR-1 has nothing to attribute supplies to without a
supplier GSTIN. The codebase already had a component built for this exact
refusal (`DocumentError.jsx`). The distinction was implemented; it just was not
written down.

---

## 2026-08-31 — SUITE 05 (GANIT) 13/24, AND WHY `ganit_payments` IS EMPTY

🔴 **ONE CONTACT TOOK FIVE TESTS DOWN.** 05.09 did not fail on payments — it
failed on *"05.06 must run first — Expected 45, Received 0"*. No invoices
existed. And 05.06 failed reporting what reads as a product defect:

> picking the customer did not adopt their company

⚠ **IT IS NOT ONE, AND I ALMOST FIXED THE WRONG THING.** `pickCustomer` is
correct, the API returns `client_id`, and **80 of Unicode's 81 contacts have
one**. The single exception — `S16 Contact 202608302221` — was created by
**Suite 16** at 22:26 the night before, and `/v1/graha/contacts` is
`ORDER BY created_at DESC`, so the newest contact is `contacts[0]` and
`createInvoice(1)` reached for exactly it. A contact with no employer has no
company to adopt. **The form was right.**

One row left by an unrelated suite blocked 45 invoices, and 05.07, 05.08, 05.09,
05.13 and 05.19 then failed on "05.06 must run first". Fixed in the SPEC (filter
to contacts that have an employer) — not by deleting the row: the suites share
one org by design, and a test that only passes when nobody else has run is not a
test.

🔴 **THE CORS ERRORS WERE THE ANALYTICS 500 IN DISGUISE.** 05.18 reported
`/api/v1/analytics/views` "blocked by CORS policy". Probed live: the OPTIONS
preflight returns **200 with `allow-origin: https://app.kartavaya.com` on BOTH
hosts**, and an unauthenticated GET returns **401 WITH the header**. CORS is
fine. An unhandled 500 escapes the middleware and reaches the browser bare,
which Chrome reports as a CORS violation — and that endpoint was 500-ing for
every module on the `KeyError: 'metric'` fixed earlier today. **The analytics
fix is bigger than it looked**: a console error on every Finance settings screen,
not just an empty views bar.

🟡 **AN EMPTY FILTER IS NOT AN EMPTY TABLE.** "metered-usage has 18 rows on the
wire and paints none" is a harness false positive — the tab defaults to
*Unbilled* and live data is 18 rows, **18 invoiced, 0 unbilled**. But the empty
state read "No usage entries" while eighteen sat one dropdown away. Saying the
WRONG words is the version of that failure nobody checks for, because the reader
stops looking. Copy is now filter-aware.

**Not defects:** 05.15 — *"THE OUTBOUND FENCE IS DOWN … NOT sent,
deliberately"* is the fence working as designed.

**Still open:** 05.05 (`ExpensesTab` renders no file input while
`ExpenseCreate.receipt_urls` is API-writable — an orphaned capability, and a
feature rather than a repair) and 05.17 (a TDS challan refused, not yet
diagnosed).

---

## 2026-08-31 — SUITE 20 RUN AGAINST PRODUCTION: 11/16, AND 3 OF THE 5 NOW CLOSED

`suite20.config.ts` — **11 passed, 5 failed, 14.0m**, 187 screens crawled, 959
rows measured. **0 uncaught exceptions and 0 console errors across all 187.**

⚠ **THE SUITES DRIVE THE DEPLOYED APP.** Every fix below is in the working tree
and **NOT DEPLOYED**, so a re-run today still reports the old numbers. That is
the fix not being shipped, not the fix failing.

| Test | State |
|---|---|
| 20.03 no screen paints a UUID | 🔧 **FIXED** — `folder_label` |
| 20.04 no native date control | 🔧 **FIXED** — the month mode |
| 20.05 every row on `--row-h` | 🟡 scan fixed; **6 genuine remain, not guessed at** |
...
🔴 **TWO DRAG HANDLES DID NOTHING WHILE PROMISING THEY WOULD.** `.ktabs__grip`
and `.kcols__grip` are `<button>`s, and `@hello-pangea/dnd` refuses to start a
drag whose source event targets one of its `interactiveTagNames` — so neither
mouse nor keyboard ever began, while the grips are labelled *"Space picks it up,
arrows move it, Space drops it."* Column order is a SAVED preference, so it
could not be reordered by any means the UI offered. **The identical guard that
stopped every kanban card until 2026-08-29**; `KanbanView.jsx:638` already had
the escape hatch and the reasoning.

🔴 **7 OF 10 SAMPLED SCREENS WERE SILENT WHILE LOADING.** `vetana#payslips` is
the sharp case: a skeleton IS drawn, so it looks busy to an eye and says nothing
to a screen reader. The three that passed had each hand-written a
`SkeletonRegion` — **the contract lived in a wrapper every call site had to
remember**, and the shorter, obvious spelling was silent. The primitives now
carry it, with a context so the screens that were already correct do not begin
double-announcing. 11 tests, 5/5 mutations killed.

🔴 **`graha#documents` WAS THE ONLY SCREEN OF 187 PAINTING A UUID** — the R2
storage key `crm/<client_id>/documents`, printed raw in both the folder picker
and the Folder column. `check-rendered-ids.mjs` stayed GREEN over it. Now one
SQL-computed `folder_label` for both endpoints.

🟡 **20.05's scan was drowning its own signal**: of ten screens named, FOUR were
inline editor rows (`<tr><td colspan=N><form>`), where holding a form to a 50px
row token asks the wrong question — now excluded by SHAPE. Six are genuine and
**left open rather than guessed at**: the obvious remedy is a global
`white-space: nowrap` across 187 screens, which is the shape of incident this
repo keeps recording. It needs the offending cell measured first.

**Verified:** frontend 3,278 tests / 204 files green, `npm run build` green,
20/20 gates green; backend 2,829 green across every touched module.

→ Full detail in `docs/plans/PROGRESS.md` under this date.

---

## 2026-08-31 — ✅ THE NATIVE `month` CONTROL IS GONE (SUITE 20.04 CLOSED)

CLAUDE.md bans native date-family controls. `Field.jsx` forwarded `date`,
`datetime-local` and `time` to `DateInput` and **not `month`**, so five screens
still emitted the browser's own widget. **Suite 20.04 failed on this
deliberately**, naming the fix as a feature instead of excusing it into a green:
*"closing it means giving `DateInput` a month mode"*. That decision has now been
taken and built.

Three of the five were Vetana, where `manav/BonusTab.jsx:56` had already written
down the cost: the month must match `vetana_payroll_runs.month` EXACTLY, and a
wrong one *"does not fail, it files the award against a month no payroll run
will ever look at, and the person is simply not paid."*

**Built:** `components/ui/MonthGrid.jsx` (CalendarGrid's own markup and classes,
one tab stop, arrow/Home/End keys, `min`/`max`); `DateInput type="month"` with
an "Aug 2026" label and a This month / Last month / Clear row; `month` added to
`Field.jsx`'s `DATEY`; **five screens migrated** (`vetana/PayrollTab`,
`PayslipsTab`, `StatutoryTab`, `ganit/StatsTab`, `manav/PerformanceTab`), each
`<label>` becoming a `<div>` + `aria-labelledby` because DateInput renders a
BUTTON and a label cannot label one; `_helpers.ts::setMonth()`; **10 spec
locators** repointed across two spec files.

⚠ **A SCREENSHOT CAUGHT A BUG THAT 15 GREEN TESTS COULD NOT.** The first
render laid the months out **7 across** — `.pk__grid` is the calendar's
`repeat(7, 1fr)` week and `.pk__grow`/`.pk__gcell` are `display: contents`, so
the JSX row grouping carries no layout weight at all. Every unit test passed
over it: the DOM order, roles, labels and emitted value were all correct, and
only the painted result was wrong. Nobody writes `expect(monthsPerRow).toBe(4)`
— which is the argument for looking at the thing.

**Verified:** 15 vitest tests; driven live in the browser (mounted via the dev
server with no repo changes — Jun selected, Aug carrying the today dot,
Sep–Dec disabled by `max`, console logging `EMITTED: 2026-03` on a pick);
`npm run build` green; **all 20 `npm run check` gates green**; all four modified
spec files parse under `playwright --list`.

→ Full detail in `docs/plans/PROGRESS.md` under this date.

---

## 2026-08-31 — FIVE MORE MONEY/RELIABILITY DEFECTS, AND TWO GUARDS THAT HAD GONE RED

| | Defect | Measured |
|---|---|---|
| 🔴 | **Vikray dashboard reported 3.2x the real revenue** — drafts, credit notes (added instead of subtracted) and soft-deleted rows all counted | **817,016.00** shown vs **257,696.00** true |
| 🔴 | **Same inflation on GST threshold turnover** — the figure checked against registration/audit limits, whose docstring calls it a FLOOR | same 817,016 → 257,696 |
| 🔴 | **Billing sweep billed DEACTIVATED orgs**, and one org's failure silently truncated the run — earlier orgs stayed advanced, a re-run would advance them **twice** | now isolated, reported, tick fails |
| 🔴 | **Unbounded memory leak** in the global write limiter — one entry per client IP, never removed, for the life of the worker | now swept once a minute |
| 🟡 | **Payables headline disagreed with its own aging breakdown** | latent — **0 cancelled bills today**, so the two agree VACUOUSLY |

⚠ **TWO ANTI-VACUITY GUARDS WERE THEMSELVES RED**, which is worse than the bugs
above because it means other checks were unverified while they sat green:

- `test_cron_fails_loudly.py::test_the_scan_can_see_the_handlers_at_all` — the
  test that exists to stop that whole file passing vacuously. Red since
  **2026-08-29** (`run_recycle_bin_purge` added, count not updated), so every
  `assert not ...` under it proved nothing for two days.
- Four analytics tests subscripted `w["metric"]` on a two-shape layout — the
  SAME false assumption that made `/v1/analytics/views` answer 500. The
  endpoint's contract was unheld by the file whose job is to hold it, exactly
  when it broke.

**Overturned — P7 "toast deadlock" is NOT a defect.** `.k-toasts` is
`pointer-events: none` and 320px in a corner; `.tst { pointer-events: all }`
re-enables only the card so its dismiss button works, and z-520-over-modal-420
is the documented ladder. My earlier note read the card rule without its
container. Already correct in HEAD.

**Verified:** 3,536 backend tests pass, 0 failures. 24 new tests. Mutation
matrices 6/6, 6/6, and 4/6 — the two survivors on the last are recorded in the
test file: one is an EQUIVALENT MUTANT, and the other's test **passed under the
mutation**, so it was deleted rather than kept as a green check proving nothing.

🔴 **FOUND, NOT FIXED — `ganit_payments` holds ZERO rows.** No payment has ever
been recorded, so `collected`, receivables ageing, cash position and dunning are
all unexercised against a real payment. Not a code defect; the largest remaining
coverage hole, and per CLAUDE.md a table at 0 rows is two unknowns, not one.

→ Full detail in `docs/plans/PROGRESS.md` under this date.

---

## 2026-08-31 — GST WAS CHARGED ON THE PRE-DISCOUNT VALUE, AND A FINAL TAX INVOICE CARRIED IT

**Fixed in code and in the data. Six implementations of one defect.**

s.15(3)(a) CGST Act excludes an invoice-recorded discount from the transaction
value. Every site applied the per-line `discount_pct` correctly, then took the
flat order/invoice discount off **the total only** — never off the base the
tax was computed on. Found by a live query.

⚠ **SUITE 10 PASSED THE WHOLE TIME.** Nothing asserted the tax *base*, only
that the figures were internally consistent — and they were, consistently
computed on the wrong value. The anti-vacuity lesson again: a check that
reconciles a document against itself cannot see a wrong input.
...
All Unicode Group, seeded 2026-08-30; **−₹3,722.99** of output tax over-stated.
Aekam Inc holds no discounted row. Rows corrected, reversal captured first;
every one now taxes at exactly its own rate on the net (18.00 / 18.00 / 5.00 /
18.00 / 0.00 / 15.46, the last being the correct 18-and-5 blend).

**Fixed:** `routers/vikray.py`, `routers/ganit.py` (the tax invoice itself),
`services/purchase_orders.py`, and all three client previews —
`ganit/InvoiceForm.jsx`, `vikray/_shared.jsx`, `procurement/_shared.jsx`. The
previews agreed with the wrong server, so nothing on screen looked amiss;
`vikray/_shared.jsx` even carried a docstring **excusing** the client/server
disagreement as a tolerance. That caveat is deleted — what it excused was
₹900 of real money, not a rounding order.

**A second defect found on the way:** CGST/SGST were split by rounding *both*
halves up per line, so on an odd paisa `CGST + SGST > tax` and the same goods
totalled more intra-state than inter-state. Now an exact split in all four
places. `purchase_orders` reports a tax discrepancy on any mismatch, so this
would have surfaced as a spurious exception on a correctly matched PO.

**A test that had been RED:** `test_order_to_invoice_carries_the_cost_verbatim`
raised `KeyError: 'salesperson_id'` from a stub row that had drifted from a real
column — so the cost-carried-verbatim invariant was not being checked at all,
by the test that exists to check it.

**Verified:** 1,228 backend tests green; 11 new Python + 8 new vitest tests;
mutation-proved **7/7** server-side with every kill attributed to a named test,
4 kills client-side. `npm run build` and all 20 `npm run check` gates green.

→ Full detail, including why a FINAL invoice was corrected in place rather
than by credit note, in `docs/plans/PROGRESS.md` under this date.

---

## ⚠ 2026-08-30 LATE — READ THIS FIRST

**THERE IS NO STAGING.** One system, one Supabase project, one schema `public`,
one branch: **`main`, and `main` IS production**. The host labelled `staging` is
a second, staler front door onto the same database — it buys `OUTBOUND_MODE=dry`
(mail, push and social, **not data**) and nothing else, on a backend **only as fresh as the last catch-up merge** (**0 commits
behind**. A write through it is a production write. Full detail in `CLAUDE.md` →
*The one dangerous fact*, and in `docs/plans/PROGRESS.md` under this date.

**Three privilege/exposure defects were found and fixed LOCALLY. All three are
still live in production until `main` deploys:**

| | Defect | State |
|---|---|---|
| 🔴 | **`server.py` granted `platform_admin` on every boot** to any user with `users.role='admin'` — a per-org column read as a platform-wide one. Two e2e accounts, incl. the OWNER fixture, would have gained god mode on the next restart | fixed + 3 tests, mutation-proved |
| 🔴 | **`POST /roles/assign` allowed self-granting a platform role** — falsifying the invariant that only `platform_support` may hold a support session | fixed + 5 tests, mutation-proved both ways |
| 🔴 | **The staging host served `/openapi.json` unauthenticated** — HTTP 200, 1,022,070 bytes, the full API map of a payroll product | fixed, 56 tests pass |

**Blocker 1 is CLOSED.** The OWNER credential is restored; `--project=setup`
passes **3/3** with owner, approver and godmode as three genuinely distinct
accounts for the first time.

**Gate P2 is MEASURED, and it carries an ABORT.** 300 base tables, 249 with
`org_id`. Delete scope is **1,384 rows**; Aekam Inc holds **4,630** and must
survive. ⚠ **`users` cannot be org-scoped, and four people — including the
owner's own login — belong to both a TOUCH and a NO-TOUCH org, so a
"delete the users of these orgs" step locks the owner out of Aekam Inc.**
⚠ **3,007 rows carry `org_id IS NULL`** and are invisible to every org-scoped
statement — the wipe will report zero for the org while they remain.

**Still open:** `outbound_log` never records the From address; a third
platform-role write path is unfound; `pyjwt` is not yet at 2.13.0; the
`staging.kartavaya.com` DNS step in `DNS-AND-SUBDOMAINS.md:245` **must not be
executed** — it converts "test against staging" from fail-closed to fail-open.
...
| Skills / dock / Sahayak | 69–72 | 🟡 **stale — corrected 27 Aug.** The Due tab is NOT dead: Phase 4.5 fixed it (the `income_tax` typo was worth 22 rows; Finance 7 → 13). `skill_finding_ack` is NOT 0 rows: Phase 4.3 took it **0 → 1** through the deployed endpoint and re-running the skill then returned 2 findings instead of 3. **Re-measured 2026-09-01: ack wiring is 31/78, not 32** (`ACK_WIRING` keys against `public.hub_skill_templates`), and the shelf is 78 templates all active / 234 grants / 3 orgs / **`hub_skill_runs` = 1** / `skill_finding_ack` = 0. ⚠ **THE SHELF HAS NEVER RUN BY ITSELF AND THE REASON IS ONE EMPTY COLUMN**: `/cron/skills` selects on `trigger_config->>'type' = 'cron'` and `trigger_config` is NULL on all 78, so the sweep has matched zero rows every 15 minutes since it was built; `last_run_at` is NULL on all 234 grants. The arming control (`PUT /v1/hub/skills/templates/{id}/schedule`) exists and has never been used. ✅ **Migration 261 lands `used_by` + `when_to_run`, backfilled 78/78** and rendered on the Catalog card, the drawer and both assigned shelves — a card now says who it is for and when to run it, which is the sentence that tells somebody what schedule to arm. Deliberately NOT added: a `next_actions` column — the only verb on a finding today is Dismiss, and a card advertising "creates tasks" would sell a button that does not exist. ✅ **ARMED 2026-09-01 — migration 262 puts TEN free checks on a schedule, the first unattended runs in the product's history.** All ten are free, model-free, write-free, org-scopable, not subject-bound and already in `ACK_WIRING` (that last one is not optional — an armed skill with no dismiss path manufactures wallpaper on a timer). Daily: Approvals that sit, What we are waiting on. Weekly: Duplicate vendor bills, MSME 45-day clock, Money in / invoice unpaid. Monthly: WIP ageing (3rd), Retainers that stopped billing (5th), Amend before you file (12th), Attendance exceptions + Statutory records gate (25th). 30 active grants. ⚠ **THE SWEEP IS DAILY AT 01:15 UTC, NOT EVERY 15 MINUTES** — `run_skills`' docstring said the latter for its whole life, and only `cron-daily-prod` (`15 1 * * *`) reaches `/cron/skills`. So `EXTRACT(HOUR FROM now())` is always 1 and the first draft's `hour_utc` 3/4 would have armed five skills that could NEVER fire — stored, rendered on the card, silently dead. Caught before it shipped; `hour_utc` dropped, `schedule.SWEEP_HOUR_UTC` now refuses an unreachable hour at the door, and the docstring is corrected. ✅ **PROOF IN — 2026-09-02 01:16:10 UTC, the shelf ran itself for the first time in the product's history**: 15 runs (5 interval templates x 3 orgs), all `completed`, **0 credits**, every one carrying outputs; `hub_org_skills.last_run_at` **0 → 15**. Findings were real: Unicode Group had two documents at chase rung 1, and the three approval ladders returned 9 rows / 15 escalation targets; two skills came back clean, which is rendered as a finding not a blank. ⚠ **Watch `hub_org_skill_runs`, NOT `hub_skill_runs`** — the latter is keyed on `client_skill_id` and stays flat at 1 while org runs land beside it. ✅ **263 arms ELEVEN MORE — 21 templates / 63 grants.** ⚠ **And it found the real constraint: of 49 free unarmed templates, 12 clear every condition and **37 fail exactly one — they are not in `ACK_WIRING`**. Not credits, not writes, not scheduling. Only 32 of 78 skills can have a finding dismissed, so finishing that wiring is now the highest-value work on the shelf. (`ACK_WIRING` is **32**, not 31 — a grep of `[a-z_]+` drops `check_194q_approaching`.) Quotation expiry chase stays unarmed: its card says "Once quotations exist". ⚠⚠ **AND THAT "37 BLOCKED" READING WAS WRONG — corrected 2026-09-02.** `skill_ack_wiring.py` states the unwired skills were **measured and excluded** (write skills, aggregates, narratives, work lists, DETECT scorers, and period-scoped statutory/calendar skills whose findings expire). Checked one at a time against the categories using the real output shapes in `hub_org_skill_runs`: **36 of 37 were correctly out**. Ack wiring is NOT the constraint on arming them — most need no ack at all. ✅ The one genuine omission, `check_dead_gst_slabs`, is now wired (33 of 93), with 17 tests and two mutation checks. ✅ **264 arms 21 MORE — 42 templates / 126 grants**, and moves the bar from "must be in ACK_WIRING" to "in ACK_WIRING **or** carrying a recorded reason it need not be", enforced by a guard: a period-scoped finding has nothing to acknowledge because the period closes it. **17 stay unarmed with a reason each** — report-nothing-yet, event-driven, one-off, a momentary guard, or drafts nobody can send. ⚠ **Client filing calendar is in that set: `client_obligations` holds ZERO rows**, so it is waiting on a screen, not a schedule. ✅ **THE SCREEN IS BUILT (2026-09-02)** — five endpoints on `routers/graha.py` plus `ObligationsSection.jsx` in the client detail aside, with the form warning that **9 of the 16 obligations cannot be dated** (QRMP most sharply) derived from the same `_NO_CALENDAR_RULE` the skill reads. 25 tests incl. a live half that plans every statement. ✅ **AND THE LOOP IS CLOSED.** First row written **through the deployed endpoint** (201) on 2026-09-02; both guards refused correctly live (409 duplicate, 400 unknown key). The register lit up at once. The calendar did not until the row was backdated — it reads obligations AS AT the period it dates, and the form defaulted to today, which is a real trap now fixed in the hint. It then produced **GSTR-1 due 11 Sep and GSTR-3B due 20 Sep but WORKED BY 18 Sep**, because the 20th is a Sunday. Migration 265 arms both on the 1st: **44 templates / 132 grants**. **4 obligations across 3 clients in 2 orgs** — UK AekamINC has ZERO clients so nothing can attach there, and Aekam's two clients got no GST row because neither has a GSTIN (that would invent a registration). ⚠ **NEW FINDING: income tax cannot be dated after 1 Apr 2026** — the 1961-Act rows in `statute_calendar` were end-dated at the repeal and no Income-tax Act 2025 successors were seeded; `tds.statement.*` have never carried a due day at all. ✅ **PARTLY FIXED by migration 266 (researched, not recalled)**: `tds.deposit.monthly` and the four advance-tax rows seeded with a `source_ref` each, plus `s.397(3)(b)` on the four statement rows. Aekam's calendar now prints **TDS deposit for Aug 2026 due 7 Sep**; undated filings fell 4 → 2. ✅ **AND THE QUARTERLY STATEMENTS ARE NOW DATED TOO — migration 267 + the resolver change 266 said was needed.** Q1-Q3 are +1 month and **Q4 is +2 (31 May)**, which one `due_month_offset` cannot say, so 266 left four filings printing a form number and no date. `due_overrides` is a nullable jsonb keyed by the **period-end month** carrying only what differs — `{"3": {"month_offset": 2}}` for the statements, `{"3": {"day": 30}}` for the deposit — so a monthly rule and a quarterly one use one column and one code path. Proven live on the deployed calendar: **Form 140 for the quarter ended 31 Mar 2027 is due 31 May 2027**, and the quarter ended 30 Sep 2026 is due 31 Oct with **work_by 30 Oct** because the 31st is a Sunday. ✅ **The March deposit is fixed as well and it was the older bug** — TDS deducted in March is payable by **30 April**, not 7 April, and had been three weeks early since the 1961 row was first seeded. Undated income-tax filings for those clients: **2 → 0**. ⚠ **ONE RESOLVER NOW, WHERE THERE WERE TWO.** `_due_date_from` existed byte-identically in `gst_year` and `client_register`, with `delta_and_provenance` importing the first under a note that a third copy would be a third chance at the GSTR-9 bug that already shipped (nine months early, beside a statute citation). Canonical `services.statute.due_date_from`; both former copies are delegates; a test asserts every caller is the same object. 266’s guard is **replaced, not deleted** — a quarterly row may carry a due day ONLY if it also carries the period-end-March exception. ⚠ **A SEVENTH TEST WAS SATISFIED BY ITS OWN SHAPE, AGAIN.** Six mutations of the resolver all turned the suite red; `test_an_offset_override_clears_an_absolute_month` stayed green with the line it named deleted, because the offset branch wins first and the clearing was unobservable. The dead line was REMOVED rather than covered. 32 tests, live half resolves 17 real dates. ✅ **AND THE 1961 ROWS ARE NOW SEEDED TOO — migration 268.** The gap had a CURRENT example: Q4 FY2025-26 covers the quarter ended 31 Mar 2026, before the repeal, so it resolved against a 1961-Act row and came back blank; it was due 31 May 2026. Live now: **form 26Q, quarter ended 2026-03-31, due 2026-05-31, work_by 29 May** (the 31st is a Sunday, the 30th a Saturday). ⚠ **TCS IS NOT TDS UNDER THE 1961 ACT AND A SOURCE SUMMARY SAID IT WAS** — rule 31A is the **31st** (24Q/26Q/27Q) and rule 31AA is the **15th** (27EQ), because Notification 30/2016 moved 31A and never touched 31AA; only the 2025 Act put them on the same day, which is why the Form 143 row is on the 31st. The first search returned "these apply to 24Q, 26Q, 27Q and 27EQ equally" — seeding it would have put every TCS statement sixteen days late beside a rule citation. A fourth candidate (30 April) traced to rule 37CA, a different obligation. **Each key is now TWO versions**, because the old rows start at 1962-04-01 with `effective_from_exact = FALSE` (a floor, not a commencement) and the dates differed BY DEDUCTOR TYPE before June 2016: 1962→2016-06-01 stays undated, 2016-06-01→2026-04-01 is dated. A quarter ending in 2015 still returns no date, asserted not assumed. ⚠⚠ **AND YESTERDAY’S "ONE RESOLVER, WHERE THERE WERE TWO" WAS WRONG — THERE WERE THREE.** `firm_flow.py` held a third copy, found by grepping the tree. `test_there_is_exactly_one_resolver` could not have caught it: it NAMED the three modules already known and already fixed, so it only restated the work — an assertion satisfied by its own shape, in the test written to prevent that, one day after mutation testing found the same fault in a sibling test. The copy was already wrong: it never read 267’s `due_overrides`, so the firm flow and the client calendar printed DIFFERENT dates for the same obligation in March (7 Apr vs 30 Apr). The replacement WALKS `services/` and reads the source, with an anti-vacuity floor on the file count, and was watched going red before the fix. ⚠ Also recorded: `IF overlaps > 0` is a SYNTAX ERROR — `OVERLAPS` is a reserved SQL operator and the error names the `>`. ⚠ **THE SWEEP THE THIRD RESOLVER ASKED FOR RAN ON 2026-09-03, AND IT WAS NOT A GREP.** Function bodies were normalised to an AST across 331 production `.py` files and 893 shipped JS/TS files, so a renamed copy still collides: **35 identical-body groups in Python, 32 in shipped JS**, plus 141 same-name-different-body groups. ✅ **`_statute_note` was `_due_date_from` again, one function down — FIVE copies in the same four files, and `grep -rl _statute_note tests/` returned NOTHING.** It prints the citation beside every statutory date. Four copies were byte-identical; `firm_flow`'s had already drifted to `or row.get("authority")`, and `authority` is a ROUTING SLUG — `income_tax`, `gst`, `epfo`, `esic` — so that copy would print "Monthly TDS deposit (income_tax)" where a section reference belongs. Unobservable, and that is how it survived: `statute` is non-null on **70 of 70** rows (measured), so nothing could tell the two apart until a row is seeded without an Act name. Canonical `services.statute.statute_note`; the drift resolved toward the four, not the one. ✅ **`_fy_of` collapsed too** — three copies, and `statute.fy_bounds`, its documented inverse, was already imported by two of them; `delta_and_provenance` no longer reaches into `gst_year` for it. 25 tests incl. a live half over the real calendar; three mutations each turned it red, and the ratchet now WALKS `services/` for both helpers rather than naming modules. ⚠ **STILL OPEN, and it needs a decision rather than a patch: `_month_bounds` / `_period_bounds` — ten copies in `services/skills/data/`, ONE NAME OVER TWO CONTRACTS.** Six return an inclusive last day, four return an exclusive first-of-next, and `_period_bounds` alone is three files split two-to-one. Pairing `<= end` with a half-open bound drops the last day of the month, which `ganit_ops`' own docstring warns about by name. Measured by executing all ten against the same inputs — which also **corrected the reason three of them give for existing**: the `'2026-00'` guard they document at length is DEAD in every copy, because `date(y, 0, 1)` raises first. ✅ **Frontend, same sweep**: `lib/inr.js` opens "one implementation" and `lib/utils.js` held a byte-identical twin of its `inrShort` under the name `formatINR` (8 consumers vs 1); Dristi and Prachar each declared the same `DataTable`+`Td` adapter, cross-referenced in both directions with a note saying neither could import the other — true, and neither considered the third home both already imported from, now `components/ui/moduleTable.jsx`; `thisMonth` ×4 (**one of them UTC**, so it names the previous month for an IST user before 05:30 on the 1st — left in place, renamed `thisMonthUtc`, because which one an invoice wants is a product decision), `monthRange` ×2 (both built their bounds from the UNRESOLVED argument, so a no-arg call returned `undefined-01`), `stamp`/`shortStamp` ×2 (hub's carrying the docstring "one implementation"), `Shim` ×3 (each copy holding the same a11y fix in a comment), `refusalMessage` ×4 — **and all four of those read only the string and `{message}` shapes, so a 422's field names fell through to the generic fallback: the exact defect `lib/apiError.js` exists for and documents**. Also `ringsOf`/`boundsOf` (the GeoJSON `[lng,lat]` → SDK `{lat,lng}` swap, written out twice), `text`/`stateOf` ×3, `useClientOptions` ×2. 3,431 frontend tests and 20 gates green; the `moduleTables` test was rewritten to assert the shared adapter and to FAIL on a re-declaration. ✅ **AND THE OPEN ITEM IS CLOSED — 2026-09-04, and the walk found FOUR MORE COPIES THAN THE ENUMERATION DID.** `_month_bounds`/`_period_bounds` is now `timeutil.month_days` (inclusive, pair with `<=`) and `timeutil.month_window` (half-open, pair with `<`). **Two named functions, not `inclusive=False`**: a bare `False` in an argument list says nothing about what it buys and picking it wrong does not raise — it silently answers about a window one day off — whereas `start, before = month_window(p)` beside `<= $3` reads wrong at a glance. ⚠ **AGAINST A `timestamptz` ONLY THE HALF-OPEN FORM IS CORRECT** (`<= last_day` drops nearly all of the last day); against a `date` column both work, which is why both ship and why `varta_consent` has its own test pinning it. **No query changed** — every pairing was checked against the operator in its own SQL first, and all ten were already right, so this is hygiene and was priced as such. ⚠ **THE ENUMERATION SAID TEN; THE RATCHET FOUND FOURTEEN.** `itc_reversal._period_end` was an eleventh in disguise (`month_days(p)[1]`) and its own docstring said where it belonged; `services/gst_period.py` held THREE more — the helper plus the same rollover inlined twice, once ten lines below it. Two `period_bounds` in `commission.py` and `platform_proration.py` are NOT copies (cadence, not a month) and are exempted by name with their signature rather than by narrowing the pattern. ⚠ **AND IT CORRECTS YESTERDAY'S OWN FINDING.** "The month-0 guard is dead in every copy" holds for the ten, which all built the month's first day and so raised anyway — but **`_period_end` computed only the END, so `'2026-00'` really did return 2025-12-31, a cutoff in the wrong YEAR**, and there the guard was load-bearing. Kept, with the unguarded body preserved in a test as the anti-vacuity proof. ⚠ **A NEW ASSERTION WAS SATISFIED BY PROSE, caught by mutation**: the check that `gst_period.period_bounds` still delegates was `"month_window" in body`, and the function's own COMMENT contains that word — re-inlining the rollover underneath left it green. It parses an `ast.Call` now. Six mutations, all red. 59 new tests; two existing ones corrected — one string-matched an exact import line, one asserted the None contract that deliberately moved. ✅ **AND THE BILLING PERIOD IS NOW IST, ON BOTH SIDES — 2026-09-04.** `credits.current_period()` was UTC, so for a product whose every customer is an Indian firm the billing month rolled over at **05:30 IST**: a charge incurred at 02:00 on the 1st was booked to the month that had already ended, and **on 1 April to the previous FINANCIAL YEAR** — which in a product Indian CAs use to close their own books is not a rounding error. Nothing raised; both months looked plausible. ⚠ **THE REST OF THE PRODUCT ALREADY AGREED**: `outbound._today_keys` has always computed its daily and monthly email caps "in IST for period boundaries", so billing was the outlier, not the change. **Measured before: 8 rows across `org_billing_lines` and `invoice_billing_lines`, ZERO whose `period_start` disagreed with what IST would give** — forward-only, no migration, nothing re-dated. `IST` had **three definitions** (esign_signed_doc, push_service, and inlined in outbound as a `+5:30` add that leaves a datetime whose tzinfo LIES) and was about to have a fourth; there is one now, `services/clock.py`. The frontend's three UTC copies — InvoiceBuilder, BillingLinesBlock, BillingUsageSection — are one `lib/dates.currentPeriod`, and the usage dropdown's `Date.UTC(y, m - i, 1)` walk (correct arithmetic on the wrong clock) is integer arithmetic off the IST anchor. `skills.timeutil.utc_now` stays UTC **deliberately**, with a test that says so: an INSTANT is UTC, a PERIOD A PERSON NAMES is IST. ⚠⚠ **AND MY OWN RECOMMENDATION WAS WRONG TWICE, BOTH CAUGHT BY THE OWNER AND BY MUTATION.** First I proposed making InvoiceBuilder alone read local time — which would have desynced one form from the server — because my rename the day before had stranded the original docstring above the one I inserted, and I re-read only my own. Then I called UTC "correct" when it was merely *consistent*. ⚠ **THREE OF NINE MUTATIONS ESCAPED THE FIRST DRAFT AND TWO WERE THE SAME FLAW THIS FILE'S OWN DOCSTRING WARNS ABOUT**: `assert current_period() == month_start_ist()` is green at every instant except the 5.5 hours a month it guards — green in CI, wrong in production. Both are structural (`ast.Call`) now. The third escaped by ALIASING the import past a source regex, so identity is asserted too. 9 of 9 red. ✅ **AND THE OTHER TWO CLOCK SURFACES FOLLOWED THE SAME DAY.** `utc_now().date()` was "today" in **52 places across 18 handlers**; it is `today_ist(utc_now())` now. `as_date` took the **UTC** calendar date of a `timestamptz` and takes the Indian one. ⚠ **THE TWO HAD TO MOVE TOGETHER**: fourteen handlers compute `days_between(today, as_date(row["created_at"]))`, so moving one clock alone would not have half-fixed the age — it would have subtracted two calendars from each other, wrong in a NEW way and at a different hour. ⚠ **`return_period` IS THE SHARP ONE BECAUSE ITS ANSWER IS STATUTORY**: it is "the month you have left" for a GST filing, derived from today, so on the UTC clock at 02:00 IST on 1 September it returned **JULY** — the clock said yesterday, then the helper subtracted a month, landing TWO months before the one being filed, on a screen that names sections and due dates. `coming_week_start` was worse in kind: at 01:30 IST on a Monday UTC still said Sunday, and Sunday's "coming Monday" is tomorrow — **a week out, not a day**. Live impact measured: **8 of 438 rows** across tasks / invoices / vendor bills have a `created_at` whose Indian date differs, so their computed age moves by one. ⚠⚠ **THE EXISTING SUITE WAS BLIND TO ALL OF IT BY CONSTRUCTION** — every frozen-clock fixture freezes at `datetime(2026, 8, 20, 6, 0, utc)`, which is **11:30 IST**, the middle of the Indian working day, where the two clocks agree about everything. 4,941 tests passed before the change and after it and proved nothing either way; the 27 new ones all freeze INSIDE the 00:00–05:30 window and state each answer as a difference from what UTC gives. ⚠ **AND THE FIRST ATTEMPT SHIPPED A CRASH I CAUGHT BEFORE RUNNING ANYTHING**: naming the helper `today` made 40 call sites read `today = today()`, which binds a local and raises `UnboundLocalError` on the right-hand side. Renamed `today_ist`. ⚠ **THEN 18 TESTS WENT RED FOR A BETTER REASON** — every fixture freezes by patching the handler module's `utc_now`, and a bare `today_ist()` reads the real clock straight past that. The ones that stayed green were the worse half: they had silently stopped freezing anything. Call sites pass the instant (`today_ist(utc_now())`) so ONE seam still freezes both clocks, and a test asserts that shape rather than trusting the convention. 6 of 6 mutations red on the first pass. ✅ **AND THE SEVEN STANDING RED TESTS ARE GREEN — 2026-09-04.** They were found by the clock sweep and confirmed pre-existing in a worktree at HEAD, not inferred. **SIX WERE STALE TESTS AND ONE WAS A MISSING CONCEPT IN A RATCHET; the production code was right in every case** — but two of them were hiding something. ⚠ **`test_report_email_wraps_pdf_AND_excel` was red because the report sender called `att.sender()` and `from_plan.resolve()` ABOVE its own `try`, on a thread nobody joins**, so the stub's missing `sender()` killed the send with no log, no `outbound_log` row and no exception anywhere a person looks — the failure-into-silence shape this codebase has now shipped six times. Both lines moved inside the try, and a new test asserts the PROPERTY (make the resolve raise; the failure must come back through `att.failed`) because adding `sender()` to the stub fixes the symptom and leaves the hole. ⚠ **`test_the_project_report_never_dunns_a_draft` was red because its ANTI-VACUITY GUARD fired**: the route had been fixed to resolve a project from `public.teams` (it used to read `public.boards`, which holds zero rows, and 404'd for every project in the product) and the fixture still answered the abandoned model — so the route 404'd, issued no invoice read, and the draft assertion certified a query it never saw. It went red rather than quietly green ONLY because that guard exists. The three `public_web_form` failures were a **KeyError: 'settings'** — a 500 on an unauthenticated public route in the test's own fixture, which had not followed the SELECT; the fixture now carries a `job_opening_id` worth leaking and a new test asserts the whole response body, so a `**settings` spread cannot pass. `test_exactly_the_named_routes_carry_the_wider_gate` did its job for the third time by name: the five client-obligation routes are written down with the reason (**the subject is a client**, which a Ganit-only org owns as much as a Graha org). `check_backup_coverage.py` is an offline DR audit that COMPARES a backup schema to `public` — the schema is the subject, not a table location — so the ratchet gained `NOT_A_TABLE_LOOKUP`, one entry, structurally separate from its debt ceiling and with a staleness check that deletes it when it stops matching. ⚠ **TWO OF SIX MUTATIONS ESCAPED AND BOTH ESCAPES WERE MY MUTATIONS BEING INVALID — and the first exposed a real hole**: the gate ratchet read only a parameter literally named `_g`, so the same dependency under any other name was gated at runtime and invisible to it. It reads every parameter now. 6 of 6 red. ✅ **AND CI IS THE FULL SUITE NOW, GREEN: 16,275 passed / 327 skipped / 3 xfailed in 5m12s** — the run that hangs locally after a heavy session. ⚠ **327 SKIPS IS NOT INCIDENTAL**: CI runs `pytest -q -rs` with `DATABASE_URL` deliberately absent, so every `live_dsn()` half skips. A green tick there is the OFFLINE half only; the live halves are the `railway run` invocations done by hand. ⚠ **The Backend job was still red on a step that is not tests** — `Dependency security audit` reported **4 NEW advisories against `pypdf==6.14.2`**, which is the library that binds the signature page onto executed documents. **Bumped to 6.16.1**, which clears all six (4 new + 2 baselined; the highest fix version among them is 6.16.1) — verified by auditing the pin on its own: *no known vulnerabilities found*. Both real paths exercised directly rather than trusting suite names: the eSign page-copy bind, and `report_delivery`'s `clone_from` + `encrypt`, which still opens with the right passphrase, still refuses a wrong one, and **still accepts an empty one** — so that module's own guard remains load-bearing. **Baseline 24 → 17**: the 5 PyJWT entries were stale (the pin had already moved to 2.13.0 and the file had not followed) and 2 pypdf ones are now fixed. Shrunk BY NAME rather than with `--write`, which re-records whatever it finds and would silently baseline anything new — the one thing the gate exists to prevent. cryptography (6) and starlette (7) are next; weasyprint (1) still has no fix. ⚠ **The local audit cannot run on this machine** — `cryptography==44.0.3` has no cp314 Windows wheel and pip-audit tries to build it from source; CI's Python 3.12 on Linux has wheels. ⚠ **Two CI jobs stay red for reasons that are not code.** E2E smoke has been red every run and CANNOT pass: `staging.kartavaya.com` → **000, no DNS** (the host CLAUDE.md says must not be created) and `kartavya-staging.up.railway.app` → **404** (the pre-rename hostname, missing the 'a'), with both tokens expired 2026-08-27. It is gated on `vars.PLAYWRIGHT_BASE_URL`, so unsetting that one variable disables it cleanly — pointing it at production instead would put a live admin session against the production database on every push, which the job's own docstring warns about. Frontend was red on a `yarn audit` **ESOCKETTIMEDOUT**, with `run-vitest-baselined: no new failures` immediately above it — the gate correctly refuses to call an audit that did not run a pass, but it has no retry. ✅ **AND THE DOMAIN TYPO IS OUT OF THE CORS ALLOWLIST — 2026-09-04.** The CI variable `E2E_API_URL` pointed at `kartavya-staging` (one letter short); corrected to `kartavaya-staging.up.railway.app`, which is **live** — `{'status':'ok','environment':'staging','outbound_mode':'dry'}`. Grepping the same typo elsewhere found it in `server.py`'s `DEFAULT_ORIGINS`: `kartavya.com`, `www.kartavya.com`, `public.kartavya.com`. ⚠ **THAT DOMAIN IS NOT OWNED BY THIS COMPANY AND ANYONE MAY BUY IT** — measured, it serves an nginx parking page titled *"Kartavya.com is for sale - Premium Domain"*. The middleware sets `allow_credentials=True`, so a permitted origin may READ credentialed responses. ⚠ **IT WAS NOT EXPLOITABLE AND THE HONEST SEVERITY IS THE UNDRAMATIC ONE**, on two mitigations that both had to hold: the `session_token` cookie is `SameSite=Lax` so a browser does not attach it to a cross-site fetch, and `COOKIE_DOMAIN` is unset so it is host-only on the API; auth is otherwise a Bearer token in `localStorage` no foreign origin can read. Removed anyway — both mitigations are one config change from gone, and an allowlist entry for a host we do not own has no upside to weigh against that. Origins **21 → 18** (first written here as 22 → 19 — recounted by parsing the literal rather than eyeballing it; `ab2cde79`'s commit message still carries the wrong number). ✅ **AND NOTHING TESTED THE ALLOWLIST AT ALL** — `grep -l DEFAULT_ORIGINS tests/` returned nothing. 34 tests now, six mutations all red. ⚠ **The sharpest of them is not about the typo**: `_VERCEL_PREVIEW_RE` is passed as `allow_origin_regex` and is **UNANCHORED**, so under `re.match` — which Starlette used in older releases — `https://kartavaya.com.attacker.example` matches `https://([a-z0-9-]+\.)?kartavaya\.com` as a PREFIX and a stranger's host is allowed with credentials. Starlette 1.3.1 uses `fullmatch`, verified against **live production**, which returned no `access-control-allow-origin` for that string. The tests drive the real app rather than reading the pattern, so a library downgrade or an added `.*` turns them red. ⚠ **STILL OPEN, and it is a removal rather than a spelling fix: the seven `kartavya-*.vercel.app` entries.** They carry the same misspelling but they are Vercel PROJECT names, not domains — there is nothing to correct them to, since `kartavaya.vercel.app` and both correctly-spelled siblings all **404**. Unlike the `kartavaya` entries these are **enforcement, not documentation**: the regex does not cover them, and production returns `access-control-allow-origin: https://kartavya.vercel.app` today. All the live ones are aliases of the **single Vercel project the company still owns** (`kartavya`, hobby team `kevalvshah03-6145s-projects`, linked to the OLD `kevalvshah/Kartavya` repo), so no stranger holds one — but `kartavya.vercel.app` serves a bare **"Create Next App"** scaffold, and deleting that project frees the name for anyone to claim, which is the `kartavya.com` shape with a cheaper trigger. `kartavya-production.akeam.vercel.app` carries a SECOND typo (`akeam`, not `aekam`), is not a Vercel URL shape, and returns 000. Left in place pending a decision that can weigh breaking a host something may still load from. ✅ **AND THE VERCEL ORIGINS ARE GONE TOO — 2026-09-04, allowlist **18 → 11**.** Seven `kartavya-*.vercel.app` entries plus the three Vercel alternatives in the regex. ⚠ **THEY WERE ENFORCEMENT, NOT DOCUMENTATION** — no regex covered them, so production really did answer `access-control-allow-origin: https://kartavya.vercel.app`. They went for a reason that is NOT the misspelling: there was nothing to correct them to (Vercel PROJECT names, not domains — `kartavaya.vercel.app` and both correctly-spelled siblings 404), and Vercel no longer serves this product at all. ⚠ **ONE OF THEM WAS A REAL EXPOSURE WAITING ON A ROUTINE CLEANUP**: `kartavya.vercel.app` and `kartavya-aekam.vercel.app` are UNSCOPED project names carrying no team suffix, so whoever holds the project holds the origin. It was still this account — one hobby project, `kartavya`, linked to the OLD `kevalvshah/Kartavya` repo, serving a bare **"Create Next App" scaffold**, not this product. **Delete that project and the name frees up for any Vercel account to claim, inheriting a credentialed grant on this API.** The trigger is not an attack, it is housekeeping. The other five carried the team slug `-kevalvshah03-6145s-projects`, which only that team can produce — dead weight, not exposure. `_VERCEL_PREVIEW_RE` → **`_ALLOWED_ORIGIN_RE`**, now `https://([a-z0-9-]+\.)?kartavaya\.com` alone: three of its four alternatives were Vercel patterns and the name pointed a reader away from the one line that actually grants `app.kartavaya.com`. ⚠ **AND `*.pages.dev` FLIPPED SIDES IN THE SAME CHANGE** — with the Vercel alternatives gone the regex covers `kartavaya.com` only, so that entry went from redundant to load-bearing; the comment says so. ⚠ **THE ENV-VAR HALF IS NOT FIXED BY A COMMIT, AND STAGING STILL CARRIES ONE.** `ALLOWED_ORIGINS = DEFAULT_ORIGINS + CORS_ORIGINS`; production's variable holds only `kartavaya.com` hosts, but **staging's is `staging.kartavaya.com,kartavya-git-staging-kevalvshah03-6145s-projects.vercel.app,localhost:3000`** — and staging is a live front door to the SAME production database. It is team-scoped so no stranger can present it, but no edit to `server.py` removes it; it needs a Railway variable write, which redeploys that service. **Left for an explicit decision.** The ratchet covers it anyway: the vercel test parametrises over `ALLOWED_ORIGINS`, not `DEFAULT_ORIGINS`, so a deploy carrying that variable fails — proven by running the suite with it set. 47 tests, five mutations all red. ⚠ **AND YESTERDAY'S OWN COUNT WAS WRONG**: "22 → 19" was eyeballed; parsing the literal gives **21 → 18 → 11**. ✅ **AND THE STAGING DOOR IS CLEAN TOO — `4481259b` on the `staging` branch, deployed and swept 2026-09-04.** ⚠ **THE STALE BRANCH WAS STILL GRANTING ALL TEN.** Railway's staging service builds `branch: staging`, not `main`, so `kartavaya-staging.up.railway.app` went on answering `access-control-allow-origin: https://kartavya.com` and `...: https://kartavya.vercel.app` — with credentials — after both `main` commits landed. **That door writes to the SAME production database.** A stale branch is not a staging environment; it is production with older code in front of it. ⚠ **AND CLEARING STAGING'S `CORS_ORIGINS` FIRST CHANGED NOTHING OBSERVABLE** — the one origin it carried (`kartavya-git-staging-...vercel.app`) was ALSO hard-coded in that branch's `DEFAULT_ORIGINS`, so the variable write was necessary and not sufficient, and the poll watching for the flip could never have gone green. Two sources, both had to go; the earlier claim that it "fixes one grant of four" is **corrected to zero**. The variable is now `staging.kartavaya.com,localhost:3000`. ⚠ **THE COMMIT IS SHAPED FOR THE MERGE THAT HAS NOT HAPPENED YET**: the CORS region was SPLICED from `main` rather than hand-edited, so it is byte-identical there and the eventual catch-up merge has nothing to resolve in it — verified by comparison, and by importing the branch's own `server.py` (11 origins, no `kartavya.com`, no `vercel.app`). The docs entries and the test file were deliberately left OFF that branch for the same reason: duplicating files that are 172 commits ahead on `main` would manufacture the conflict the splice avoids. ⚠ **AND "30 COMMITS BEHIND" WAS WRONG WHEREVER IT APPEARED — IT IS 172**, and `staging` is a strict ANCESTOR of `main` (0 commits on the left), so catching it up is a fast-forward. Corrected in `CLAUDE.md` and here. Swept after deploy: all six bad origins refused, `kartavaya.com` / `app.` / `staging.` / `pages.dev` allowed, the prefix lookalike refused, `outbound_mode: dry` unchanged. ✅ **AND `staging` IS CAUGHT UP — `0b68d4d6`, 173 commits, deployed and swept 2026-09-04.** ⚠ **PROVEN BY TREE HASH, NOT BY "no conflicts"**: the merge result and `origin/main` are both `ebcf7dd2` — the same tree object — so the staging door now serves byte-identical content. It merged cleanly because the CORS region had been SPLICED from `main` rather than hand-edited an hour earlier, which is what that shaping was for. ⚠ **THE RISK REPORT WAS RUN FIRST, because this service writes to the production database**: startup schema migrations are a **NO-OP** (the runner returns early when `public.notifications` exists — queried live, it does; 308 tables in `public`) and everything past that guard is `CREATE ... IF NOT EXISTS`; the only boot write is the stranded scraper-run sweep, which is refund-once at the database and which production's own workers run on every deploy; **the staging crons are NOT rebuilt by a git push** — they are `curlimages/curl` images on `0 0 1 1 *`, once a year — which matters because staging's `cron-nightly` calls the **retention (deletion)** endpoints against the shared database; and `REDIS_URL` is absent there but **optional by design** (`limiter.py` falls back to `memory://` rather than refusing to boot). Post-deploy health confirms all of it: `rate_limit_store: memory`, `billing_drift {platform_line: 0, credits: 0}`, `outbound_mode: dry` preserved. ⚠ **AND THE `memory` LIMITER IS NOT A WEAKENING TODAY, CHECKED RATHER THAN ASSUMED** — the per-worker store multiplies every limit by the worker count, but **both doors run `WEB_CONCURRENCY=1` with one replica**, so login's 5/min means 5/min on both. It becomes a real difference the moment staging's worker count is raised. ⚠ **A FORCE-PUSH WAS THE RIGHT MECHANISM AND WAS REFUSED BY THE PERMISSION CLASSIFIER**; verified it had NOT partially executed (staging's health payload still lacked `rate_limit_store`/`billing_drift`, which only `main`'s code returns) before doing it as a merge instead. ⚠ **THAT LEFT A COST WHICH WAS THEN PAID OFF — and this line said otherwise for twelve minutes.** The merge left `staging` carrying two commits `main` lacked, so it stopped being a strict ancestor; the entry originally recorded that as permanent. The classifier is INTERMITTENT — a second attempt at the same force-push was allowed, and `staging` is now the **same commit object** as `main` (`ba31a894`, `0 0` divergence), so the strict-ancestor property is back and the next catch-up is a plain fast-forward. **Never call a path blocked without retrying THAT path.** Discarded nothing: both staging-only commits were superseded by `main`, checked by confirming `main` holds a newer copy of every file that differed. ⚠ **AND THE "172 COMMITS BEHIND" LINE IS NOW STALE IN THE OTHER DIRECTION** — it was 30, then 172, then 0 within an hour. `CLAUDE.md` no longer carries a number there; it carries the command. ✅ **AXIOS BUMPED — 1.16.0 → 1.20.0, and it was ELEVEN advisories, not the three the truncated CI log showed.** The frontend audit gate went red on `0b68d4d6` at 14:28 with advisories published AFTER `main`'s green run at 13:54 — so `main` was green only because nothing had been pushed since. Ten axios advisories plus `form-data` CRLF injection, **two of them HIGH** (including a Node HTTP adapter reusing an inherited proxy after an interceptor changes it). ⚠ **THE HONEST SCOPE IS NARROWER THAN THE LIST LOOKS**: several are Node-adapter, proxy or `formDataToJSON` issues, and this frontend runs in a BROWSER with `axios.create({baseURL, withCredentials})` plus a header interceptor — no form serialiser, no proxy config, no `maxBodyLength`. Worth fixing, not worth overselling. Frontend `^1.15.0` → `^1.18.0` (resolves **1.20.0**); **mobile's lock already held 1.19.0 but its declared floor was `^1.8.4`**, so a fresh install could still have landed on a vulnerable version — now `^1.18.0` in both `dependencies` and `resolutions`. ⚠ **BOTH FRONTEND LOCKFILES ARE MAINTAINED** — `yarn.lock` and `package-lock.json` were last written by the same commit, and CI installs from yarn — so leaving the npm one at 1.16.0 would have handed a vulnerable tree to anyone running `npm ci` there. Both updated. ✅ **Frontend baseline 17 → 6, shrunk BY NAME**; the gate offered `--write` and that is exactly the flag that re-records whatever it finds now, silently baselining anything that appeared in the same window. All six remaining are react-router. ⚠ **AND THE LOCKFILE CHURN WAS MEASURED, NOT EYEBALLED**: `yarn upgrade` first produced a **401-line** diff touching babel, csstools and esbuild, which reads like a dependency rewrite. Parsing both lockfiles for RESOLVED versions is the check that settles it — final state is **5 version changes, 0 packages added, 0 removed** (axios, form-data, es-object-atoms, and two older proxy-agent copies that axios 1.20.0 itself depends on). ⚠ **The lockfile changed shape BETWEEN two measurements** and the second is the one that shipped — which is the whole argument for re-measuring rather than trusting a reading taken three commands ago. Verified: **build clean, 20/20 gates, 3,446 frontend tests, 846 mobile tests, mobile `tsc --noEmit` clean, audit gate exit 0**. ✅ **STARLETTE AND CRYPTOGRAPHY BUMPED — backend baseline 17 → 4.** ⚠ **STARLETTE WAS NEVER PINNED, AND THAT IS WHY IT SAT SEVEN ADVISORIES DEEP.** It arrives through fastapi, so nobody chose 0.46.2 — it was whatever the cap resolved to, and **an unchosen version is nobody's to notice**. Now `starlette==1.3.1` explicitly: the exact version that clears all seven (highest fix, PYSEC-2026-249), and the one whose `CORSMiddleware.is_allowed_origin` applies `allow_origin_regex` with `fullmatch` — the behaviour `server.py`'s unanchored pattern depends on, asserted by the allowlist tests that drive the real app. ⚠ **FASTAPI MOVED ONLY BECAUSE IT WAS THE CAP** — 0.115.12 declares `starlette<0.47.0`, so 1.3.1 was impossible without it; **no advisory names fastapi**. Took **0.138.0, not the latest 0.141.1**: both lift the cap identically (`starlette>=0.46.0`, no upper bound), and 0.138.0 is what this project's desk already runs, so it is evidenced rather than assumed. ⚠ **`cryptography 50.0.1`, BECAUSE 49 IS NOT ENOUGH** — the six fixes spread across 46.0.5, 46.0.6, 48.0.1, 49.0.0 (×2) and **50.0.0**, so the desk's own 49.0.0 would have looked like a clean bump while leaving PYSEC-2026-3552 standing. This is the Fernet key for Aadhaar numbers and R2 secrets at rest. ⚠ **CHECKED RATHER THAN HOPED: `@app.on_event("startup")` STILL REGISTERS AND FIRES** on 0.138.0 (deprecated, warns) — it is what runs the schema migrations and the stranded scraper-run sweep, so a future removal would stop both silently. ⚠ **THE REAL FINDING IS THAT `requirements.txt` HAD DRIFTED BEHIND THE DESK**: pins said fastapi 0.115.12 / starlette unpinned / cryptography 44.0.3 while the machine developing against them ran 0.138.0 / 1.3.1 / 49.0.0. Most of this bump is the file admitting what was already being exercised. Baseline shrunk **BY NAME** again, never `--write`; the 4 left are python-multipart (3, fixable) and weasyprint (1, still no fix). ⚠⚠ **AND A CLAIM MADE HERE EARLIER TODAY IS WITHDRAWN: THE LOCAL FULL AUDIT STILL CANNOT RUN.** The 09-04 pypdf entry recorded `cryptography==44.0.3`'s missing wheel as the reason, and bumping it was announced as fixing that — **wrong, and asserted from wheel availability while the run was still in flight**. It removed one blocker and revealed the next: `pydantic==2.11.1` needs a pydantic-core with no cp314 Windows wheel, so pip-audit falls back to maturin and a Rust toolchain. Same shape, different package, and the desk again runs newer (pydantic-core 2.46.4, which HAS the wheel). Bumping pydantic would buy the local audit back but no advisory names it and it is the validation layer on every route — a separate decision. Full-file verification stays with CI (Python 3.12, Linux, wheels exist); each new pin audits clean on its own. Verified: **3,345 passed / 87 skipped / 2 xfailed** across the router, service, invoice, payroll, GST, client and task suites, plus **451 passed** across encryption, Aadhaar, CORS, auth and health. ✅ **AND python-multipart 0.0.29 → 0.0.32 — THE BACKEND BASELINE IS NOW **1**, WHICH IS ITS FLOOR.** Three advisories, fixes at 0.0.30, 0.0.30 and 0.0.31, so 0.0.32 clears them with room; fastapi 0.138.0 asks only `>=0.0.18`, nothing capped. ⚠ **THIS IS THE PARSER FOR EVERY FILE THE PRODUCT ACCEPTS** — the R2 uploads in `routers/uploads.py` and `services/storage.py`, Graha client documents, Pahchan enrolment photos — and on the public web-form routes it is reachable by anyone who can post a form. ⚠ **THE ONE REMAINING ENTRY CANNOT BE BUMPED AWAY**: weasyprint 68.0 / PYSEC-2026-3412 has **NO FIX AVAILABLE**, so the file is at its floor until upstream ships something — a non-empty baseline here is not neglect, and the `_comment` now says so to stop the next reader trying. ⚠ **AND THE LOCAL TESTS PROVE NOTHING NEW ABOUT THIS ONE, WHICH IS WORTH SAYING RATHER THAN QUOTING THE PASS COUNT AS EVIDENCE**: the desk was ALREADY on 0.0.32, so every local run this session exercised it while the pin still claimed 0.0.29. The 1,701 upload/attachment/storage/esign tests confirm 0.0.32 works; they say nothing about the pin change itself. Only CI, which installs from `requirements.txt` fresh, actually tests that. **That gap — pins naming versions nobody was testing against — is the recurring finding across all four of today's backend bumps**, not a quirk of any one package. Baseline **24 → 17 → 4 → 1**, shrunk BY NAME every time |
