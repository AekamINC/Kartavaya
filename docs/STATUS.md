...
| 4 | Eight invisible-feature screens | 🟡 **three of eight landed 27 Aug — 4.6, 4.7, 4.8, all three endpoints that had no caller at all.** 4.6 the billing anchor (`PATCH /admin/billing-anchor`, owner decision 0.13 — flexible, default 1) and 4.7 pause/resume (`POST /admin/pause`, which `middleware/subscription.py:696` enforces by refusing every module) are now controls on the Plan tab of the billing console, each stating its consequence beside the button and pause taking a typed confirmation. 4.8 surfaces `GET /quota-proration` where targets are actually set — Vikray → Targets — for a salesperson who joined mid-period, with the server doing the day-count so 0.17 is not re-derived in a browser. **4.1, 4.2, 4.4, 4.5 in flight.** Nothing here is ✅ until a row appears where there were zero |
