...
#:
#: WHY THIS IS IN CODE AND NOT IN A TABLE: there is no state table anywhere in
#: this database — checked 2026-08-20 across `staging` and `public`. It is a
#: CODELIST, not a dated statutory fact like a rate or a due day, so it does not
#: belong in `statute_calendar` either.
#:
...
_GST_STATES: dict[str, tuple[str, str]] = {
...
_ALPHA_TO_NUM = {alpha: num for num, (alpha, _n) in _GST_STATES.items()}
_NAME_TO_NUM = {name.lower(): num for num, (_a, name) in _GST_STATES.items()}
...
def _norm_state(value) -> str | None:
    ...
    if text.isdigit():
        code = text.zfill(2)
        return code if code in _GST_STATES else None
    upper = text.upper()
    if upper in _ALPHA_TO_NUM:
        return _ALPHA_TO_NUM[upper]
    return _NAME_TO_NUM.get(text.lower())


def _state_view(code: str | None) -> dict:
