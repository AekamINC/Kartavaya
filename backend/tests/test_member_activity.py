...
SKIP_REASON = (
    "no live database. These statements reach `public.ganit_invoices` "
    "(including `line_items` as jsonb), `public.graha_deals` and "
...
async def test_the_two_owner_columns_are_text_and_join_public_users():
    ...
    try:
        rows = await conn.fetch(
            ...
            "    OR (table_schema = ANY(current_schemas(false)) AND table_name='graha_deals' "
...
def test_neither_section_reads_a_module_it_does_not_declare():
    ...
    assert "public.graha_" not in FINANCE_SQL
    assert "public.vikray_" not in FINANCE_SQL
    assert "public.ganit_" not in CRM_SQL
